...
public class ShiroConfiguration {
	...
	public ShiroFilterFactoryBean shiroFilter(SecurityManager securityManager) throws Exception {
		...
		try{
			...
			resources.forEach( r ->{
				filterChainDefinitionMap.put(r.getUrl(), "authc, perms[" + r.getAuthc() + "]");
			});
/*			for (SystemPermissionDto permissions : systemPermission) {
				filterChainDefinitionMap.put(permissions.getUrl().toString(), "authc, perms[" + permissions.getPermission().toString() + "]");
			}*/
		}catch (Exception e){
			e.printStackTrace();
			logger.error("============系统错误!数据初始化失败,服务失败===========");
			return null;
		}

		// 如果不设置默认会自动寻找Web工程根目录下的"/xxx.jsp"页面
		...
		filterChainDefinitionMap.put("/extra_lock1", "authc");
		filterChainDefinitionMap.put("/extra_lock2", "authc");

		filterChainDefinitionMap.put("/403", "anon");

		filterChainDefinitionMap.put("/**", "authc, perms[xxxxxxx]");  //对忘记录入数据库的url拦截
